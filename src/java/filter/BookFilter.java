...
public class BookFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session.getAttribute("newbook") == null) {
            List<Book> newbook = bookDAO.newBook();
            session.setAttribute("newbook", newbook);
        }
        if(session.getAttribute("banchay")==null){
            List<Book> banchay = bookDAO.SachBanChay();
            session.setAttribute("banchay", banchay);
        }
        
        
        OrderDAO orderDAO = new OrderDAO();
        List<OrderCart> listorder = orderDAO.getOrderBySTT();
...
//        ArrayList<Book> list = bookDAO.getPaging(1);
